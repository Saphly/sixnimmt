...
class Session:
    ...
    cards_played: dict[Player, tuple[Card, Position]] = field(
    ...
    def should_start(self):
        if len(self.players) < self.min_players:
            self.logger.debug(
                "session.should_start", succeeded=False, reason="Not enough players"
            )
            return False

        if len(self.players) > self.max_players:
            self.logger.debug(
                "session.should_start", succeeded=False, reason="Too many players"
            )
            return False
        ...
        if any(len(player.hand) > 0 for player in self.players):
            self.logger.debug(
                ...
                succeeded=False,
                reason="There are already players with cards",
            )
            return False
        ...
        if any(len(row) > 0 for row in self.board.board):
            self.logger.debug(
                "session.should_start",
                succeeded=False,
                reason="There are already cards on the board",
            )
            return False
    ...
    def should_end(self):
        if not self.started:
            self.logger.debug(
                "session.should_end", succeeded=False, reason="Session has not started"
            )
            return False
        ...
        if any(len(player.hand) > 0 for player in self.players):
            self.logger.debug(
                "session.should_end",
                succeeded=False,
                reason="There are players with cards left",
            )
            return False
    ...
    def add(self, player: Player) -> bool:
        ...
        if self.started:
            log.warning(
                "session.add_player", succeeded=False, reason="Session has started"
            )
            return False

        if len(self.players) >= self.max_players:
            self.logger.warning(
                "session.add_player", succeeded=False, reason="Session is full"
            )
            return False
    ...
    def remove(self, player: Player) -> bool:
        ...
        try:
            ...
            log.warning(
                "session.remove_player", succeeded=False, reason="Player does not exist"
            )
            return False
    ...
    def play(self, player: Player, card: Card) -> bool:
        ...
        if not self.started:
            log.warning(
                "session.play", succeeded=False, reason="Session has not started"
            )
            return False

        if self.progressed:
            log.warning(
                "session.play", succeeded=False, reason="Turn has already progressed"
            )
            return False
        ...
        if player in self.cards_to_play:
            log.warning(
                "session.play",
                succeeded=False,
                reason="Player has already played this turn",
            )
            return False
        ...
        if (self.smallest_card_player is not None) and (
            card < self.cards_to_play[self.smallest_card_player]
        ):
            self.smallest_card_player = player
    ...
    def select(self, player: Player, row: int) -> bool:
        ...
        if player != self.smallest_card_player:
            log.warning(
                "session.select", succeeded=False, reason="Not the smallest card player"
            )
            return False

        if self.selected_row is not None:
            log.warning(
                "session.select", succeeded=False, reason="Already selected a row"
            )
            return False
    ...
    def progress(self) -> bool:
        ...
        for idx, (player, card) in enumerate(
            sorted(self.cards_to_play.items(), key=lambda m: m[1])
        ):
    ...
    def start(self) -> bool:
        if not self.should_start:
            self.logger.warning(
                "session.start", succeeded=False, reason="Session is not ready to start"
            )
            return False
    ...
    def reset(self) -> bool:
        if not self.progressed:
            self.logger.warning(
                "session.reset", succeeded=False, reason="Turn has not progressed"
            )
            return False
