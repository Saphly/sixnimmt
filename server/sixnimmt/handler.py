...
async def leave(session: Session, player: Player):
    ...
    if len(session.players) == 0:
        ...
        logger.info(
            "session.delete", session_id=session.session_id, player_id=player.player_id
        )
...
async def play(session: Session, player: Player, card: Card):
    ...
    for player, card in session.cards_to_play.items():
        _broadcast(
            session,
            {"type": "play", "player": str(player.player_id), "card": card.value},
        )

    if session.smallest_card_player is not None:
        _broadcast(
            session,
            {"type": "select", "player": str(session.smallest_card_player.player_id)},
        )
    else:
    ...
    if session.should_end:
        _broadcast(
            session,
            {
                "type": "end",
                "scores": {
                    str(player.player_id): player.score for player in session.players
                },
            },
        )
    else:
...
async def start(session: Session, player: Player):
    ...
    for idx, row in enumerate(session.board.board):
        _broadcast(
            session, {"type": "init", "row": idx, "column": 0, "card": row[0].value}
        )

    for player in session.players:
        await _send(
            player, {"type": "deal", "cards": [card.value for card in player.hand]}
        )
    ...
    try:
        await _send(
            player,
            {
                ...
                "players": [str(p.player_id) for p in game.players],
            },
        )
